...
export default function Sampler({ onMousePlay, whichInstrument, midiNotes, syncMetro }) {
  ...
  // const blackNotesOctave = blackNotesArrayOctave.map((el, index) => {
  //   var includesC = false;
  //   var includesF = false;

  //   if (el.note.includes("F")) includesF = true;
  //   if (el.note === "C#4" || el.note === "C#5") includesC = true;

  //   if (!includesF && !includesC)
  //     return (
  //       <div
  //         key={el.note}
  //         onMouseDown={() => {
  //           let newArr = [...blackNotesArrayOctave];
  //           newArr[index].holdingDown = true;
  //           keyHoldingBlackOctave(newArr);
  //           synth.triggerAttackRelease(el.note, "24n", Tone.context.currentTime);
  //         }}
  //         onMouseUp={() => {
  //           let newArr = [...blackNotesArrayOctave];
  //           newArr[index].holdingDown = false;
  ...
  //           keyHoldingBlackOctave(newArr);
  //         }}
  //         className={el.holdingDown ? "blackNoteHoldingDown" : "blackNote"}
  //       ></div>
  //     );
  //   else if (!includesC)
  //     return (
